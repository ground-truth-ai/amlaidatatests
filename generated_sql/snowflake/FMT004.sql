...
FROM (
  SELECT
    CAST(GET("t1"."nationalities", 'region_code') AS VARCHAR) AS "field"
  FROM (
    SELECT
      "t0"."party_id",
    ...
    CROSS JOIN TABLE(FLATTEN(INPUT => ARRAY_GENERATE_RANGE(
      ...
      (
        GREATEST(
          ARRAY_SIZE(
            SPLIT(
              ARRAY_TO_STRING(NULLIF("t0"."nationalities", []), 'c696f582a6f84b9a926faddaaa50b715'),
              'c696f582a6f84b9a926faddaaa50b715'
            )
          )
        ) - 1
      ) + 1
