    ...
    FROM (
      SELECT
        ...
        CASE
          ...
          THEN "t2"."validity_start_time"
          ELSE "t2"."previous_row_validity_start_time"
      ...
      FROM (
        SELECT
          ...
          LAG(COALESCE("t0"."is_entity_deleted", FALSE)) OVER (PARTITION BY "t0"."party_id", "t0"."party_supplementary_data_id" ORDER BY "t0"."validity_start_time" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "previous_entity_deleted",
          LEAD("t0"."validity_start_time") OVER (PARTITION BY "t0"."party_id", "t0"."party_supplementary_data_id" ORDER BY "t0"."validity_start_time" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "next_row_validity_start_time",
          LAG("t0"."validity_start_time") OVER (PARTITION BY "t0"."party_id", "t0"."party_supplementary_data_id" ORDER BY "t0"."validity_start_time" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "previous_row_validity_start_time"
...
FROM (
  SELECT
    ...
    CASE
      ...
      THEN NULL
      ELSE "t14"."last_date"
    END AS "last_date_party_supplementary_data",
    "t14"."party_id_right",
