    ...
    FROM (
          ...
          FROM (
            ...
            FROM (
              SELECT
                ...
                LAG(COALESCE(`t2`.`is_entity_deleted`, FALSE)) OVER (PARTITION BY `t2`.`account_id`, `t2`.`party_id` ORDER BY `t2`.`validity_start_time` ASC NULLS LAST) AS `previous_entity_deleted`,
                LEAD(`t2`.`validity_start_time`) OVER (PARTITION BY `t2`.`account_id`, `t2`.`party_id` ORDER BY `t2`.`validity_start_time` ASC NULLS LAST) AS `next_row_validity_start_time`,
                LAG(`t2`.`validity_start_time`) OVER (PARTITION BY `t2`.`account_id`, `t2`.`party_id` ORDER BY `t2`.`validity_start_time` ASC NULLS LAST) AS `previous_row_validity_start_time`
      ...
      INNER JOIN (
        ...
        FROM (
          SELECT
            `t1`.`party_id`,
            `t1`.`risk_case_id`,
            SUM(CAST(`t1`.`type` = 'AML_EXIT' AS BIGINT)) AS `exits`,
            SUM(CAST(`t1`.`type` = 'AML_SAR' AS BIGINT)) AS `sars`,
      ...
      LEFT OUTER JOIN `PLACEHOLDER`.`transaction` AS `t3`
        ...
        AND IF(
          ...
          `t3`.`book_time` BETWEEN `t7`.`aml_process_start_time` - INTERVAL '365' DAY AND `t7`.`aml_process_start_time`,
