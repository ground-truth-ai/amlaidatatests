...
-- Severity: WARN
-- Description: Join date after exit date for party
-- Interpretation: If matching_rows > 0, review matching parties. Why are there parties who joined the bank prior to their exit?
SELECT
  `t4`.`total_rows`,
  `t4`.`matching_rows`,
...
FROM (
  SELECT
    COUNT(*) AS `total_rows`,
    SUM(CAST(`t3`.`join_date` < `t3`.`exit_date` AS BIGINT)) AS `matching_rows`
  FROM (
    SELECT
      *
    FROM (
