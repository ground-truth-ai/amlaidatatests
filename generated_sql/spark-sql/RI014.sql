    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          LAG(COALESCE(`t0`.`is_entity_deleted`, FALSE)) OVER (PARTITION BY `t0`.`party_id`, `t0`.`party_supplementary_data_id` ORDER BY `t0`.`validity_start_time` ASC NULLS LAST) AS `previous_entity_deleted`,
          LEAD(`t0`.`validity_start_time`) OVER (PARTITION BY `t0`.`party_id`, `t0`.`party_supplementary_data_id` ORDER BY `t0`.`validity_start_time` ASC NULLS LAST) AS `next_row_validity_start_time`,
