    ...
    SELECT
      ...
      MAX(
        IF(
          ...
          CAST('9995-01-01T00:00:00+00:00' AS TIMESTAMP),
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          LAG(COALESCE(`t0`.`is_entity_deleted`, FALSE)) OVER (PARTITION BY `t0`.`party_supplementary_data_id`, `t0`.`party_id` ORDER BY `t0`.`validity_start_time` ASC NULLS LAST) AS `previous_entity_deleted`,
          LEAD(`t0`.`validity_start_time`) OVER (PARTITION BY `t0`.`party_supplementary_data_id`, `t0`.`party_id` ORDER BY `t0`.`validity_start_time` ASC NULLS LAST) AS `next_row_validity_start_time`,
          LAG(`t0`.`validity_start_time`) OVER (PARTITION BY `t0`.`party_supplementary_data_id`, `t0`.`party_id` ORDER BY `t0`.`validity_start_time` ASC NULLS LAST) AS `previous_row_validity_start_time`
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          LAG(COALESCE(`t1`.`is_entity_deleted`, FALSE)) OVER (PARTITION BY `t1`.`party_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `previous_entity_deleted`,
          LEAD(`t1`.`validity_start_time`) OVER (PARTITION BY `t1`.`party_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `next_row_validity_start_time`,
          LAG(`t1`.`validity_start_time`) OVER (PARTITION BY `t1`.`party_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `previous_row_validity_start_time`
...
FROM (
  SELECT
    ...
    IF(
      `t14`.`last_date` = CAST('9995-01-01T00:00:00+00:00' AS TIMESTAMP),
      NULL,
      `t14`.`last_date`
    ...
    IF(
      `t14`.`last_date_right` = CAST('9995-01-01T00:00:00+00:00' AS TIMESTAMP),
      NULL,
      `t14`.`last_date_right`
    ) AS `last_date_party`
  FROM (
