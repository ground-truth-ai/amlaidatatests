    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          LAG(COALESCE(`t1`.`is_entity_deleted`, FALSE)) OVER (PARTITION BY `t1`.`party_id`, `t1`.`account_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `previous_entity_deleted`,
          LEAD(`t1`.`validity_start_time`) OVER (PARTITION BY `t1`.`party_id`, `t1`.`account_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `next_row_validity_start_time`,
          LAG(`t1`.`validity_start_time`) OVER (PARTITION BY `t1`.`party_id`, `t1`.`account_id` ORDER BY `t1`.`validity_start_time` ASC NULLS LAST) AS `previous_row_validity_start_time`
