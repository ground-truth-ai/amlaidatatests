  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          `t4`.`role`,
          `t4`.`source_system`,
          ROW_NUMBER() OVER (PARTITION BY `t4`.`account_id`, `t4`.`party_id` ORDER BY `t4`.`validity_start_time` DESC) - 1 AS `row_num`
        FROM (
          SELECT
            *
    ...
    INNER JOIN (
      ...
      FROM (
        SELECT
          `t2`.`party_id`,
          `t2`.`risk_case_id`,
          SUM(CAST(`t2`.`type` = 'AML_EXIT' AS BIGINT)) AS `exits`,
          SUM(CAST(`t2`.`type` = 'AML_SAR' AS BIGINT)) AS `sars`,
          MIN(IF(`t2`.`type` = 'AML_PROCESS_START', `t2`.`event_time`, NULL)) AS `aml_process_start_time`,
          MIN(IF(`t2`.`type` = 'AML_SUSPICIOUS_ACTIVITY_START', `t2`.`event_time`, NULL)) AS `aml_suspicious_activity_start_time`,
          MAX(IF(`t2`.`type` = 'AML_SUSPICIOUS_ACTIVITY_END', `t2`.`event_time`, NULL)) AS `aml_suspicious_activity_end_time`
    ...
    LEFT OUTER JOIN `PLACEHOLDER`.`transaction` AS `t3`
      ...
      AND IF(
        ...
        `t3`.`book_time` BETWEEN `t8`.`aml_process_start_time` - INTERVAL '365' DAY AND `t8`.`aml_process_start_time`
      )
  ) AS `t11`
  WHERE
