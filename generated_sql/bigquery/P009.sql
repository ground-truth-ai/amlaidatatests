  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        ...
        CROSS JOIN UNNEST(GENERATE_ARRAY(0, GREATEST(ARRAY_LENGTH(`t3`.`residencies`)) - 1)) AS pos
        CROSS JOIN UNNEST(`t3`.`residencies`) AS `residencies` WITH OFFSET AS pos_2
        WHERE
          pos = pos_2
          OR (
            pos > (
              ARRAY_LENGTH(`t3`.`residencies`) - 1
            )
            AND pos_2 = (
              ARRAY_LENGTH(`t3`.`residencies`) - 1
            )
