  ...
  FROM (
    ...
    INNER JOIN (
      ...
      FROM (
        SELECT
          `t2`.`party_id`,
          `t2`.`risk_case_id`,
          COUNTIF(`t2`.`type` = 'AML_EXIT') AS `exits`,
          COUNTIF(`t2`.`type` = 'AML_SAR') AS `sars`,
    ...
    LEFT OUTER JOIN `transaction` AS `t3`
      ...
      AND IF(
        ...
        `t3`.`book_time` BETWEEN TIMESTAMP_SUB(`t8`.`aml_process_start_time`, INTERVAL '365' DAY) AND `t8`.`aml_process_start_time`
      )
  ) AS `t11`
  WHERE
